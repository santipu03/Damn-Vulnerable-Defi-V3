...
describe("[Challenge] Puppet", function () {
  ...
  it("Execution", async function () {
    /**
     * LOGIC: Instead of using the approve-transferFrom pattern, we'll use ERC20Permit to execute all in one tx
     */
    /**
     * STEPS:
     *   1. Deploy the custom contract PuppetAttack (in the constructor we're going to make all the logic):
     *          - Call permit() on token to get approval of tokens of the deployer
     *          - Transfer tokens to contract
     *          - Approve 1000 tokens for the Uniswap pool
     *          - Swap 1000 tokens for ETH in the Uniswap pool
     *          - Borrow all 100000 tokens from lendingPool -- the collateral asked will be less than 20 ETH
     */

    // Anticipate the address of the attack contract to sign the ERC20 Permit
    ...
    // Deploy the contract
    await (
      await ethers.getContractFactory("PuppetAttack", player)
    ).deploy(
      token.address,
